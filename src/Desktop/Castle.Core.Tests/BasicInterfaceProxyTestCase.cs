...
namespace Castle.Core.Tests
{
	...
	public class BasicInterfaceProxyTestCase : CoreBaseTestCase
	{
		...
		{
			var methodInfo = type.GetMethod("MyTestMethod", BindingFlags.Instance | BindingFlags.Public);
...
#pragma warning restore 219
		...
		public void Should_implement_explicitly_duplicate_interface_members()
		{
			...
			var method = type.GetMethod("Foo", BindingFlags.Instance | BindingFlags.Public);
			Assert.IsNotNull(method);
			Assert.AreSame(method, type.GetTypeInfo().GetRuntimeInterfaceMap(typeof(IIdenticalOne)).TargetMethods[0]);
			var method2 = type.GetMethod("IIdenticalTwo.Foo", BindingFlags.Instance | BindingFlags.Public);
