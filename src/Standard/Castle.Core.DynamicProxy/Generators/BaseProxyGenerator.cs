...
namespace Castle.Core.DynamicProxy.Generators
{
	public abstract class BaseProxyGenerator
	{
		...
		{
			var constructors =
				baseType.GetTypeInfo().GetConstructors(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
		...
		protected void GenerateParameterlessConstructor(ClassEmitter emitter, Type baseClass, FieldReference interceptorField)
		{
			...
			var defaultConstructor = baseClass.GetConstructor(BindingFlags.Public | BindingFlags.Instance, null, Type.EmptyTypes, null);
		...
		private bool IsConstructorVisible(ConstructorInfo constructor)
		{
			return constructor.IsPublic ||
			       ...
			       constructor.IsAssembly && constructor.DeclaringType.GetTypeInfo().Assembly.IsInternalToDynamicProxy();
		}

		private bool OverridesEqualsAndGetHashCode(Type type)
		{
			var equalsMethod = type.GetTypeInfo().GetMethod("Equals", BindingFlags.Public | BindingFlags.Instance);
			if (equalsMethod == null || equalsMethod.DeclaringType == typeof(object) || equalsMethod.IsAbstract)
				return false;

			var getHashCodeMethod = type.GetTypeInfo().GetMethod("GetHashCode", BindingFlags.Public | BindingFlags.Instance);
