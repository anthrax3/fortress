...
namespace Castle.Core.Tests
{
	...
	public class BasicClassProxyTestCase : BasePEVerifyTestCase
	{
		...
		{
			return GetType().GetTypeInfo().Assembly.IsAssemblySigned();
		}
...
#pragma warning restore 219
		...
		public void ClassWithDifferentAccessLevelOnProperties()
		{
			...
			var proxy = generator.CreateClassProxy(typeof(DiffAccessLevelOnProperties), logger);
		...
		public void ClassWithInheritance()
		{
			...
			var proxy = generator.CreateClassProxy(typeof(ExtendedServiceClass), logger);
		...
		{
			var mc = generator.CreateClassProxy<MyClass>(new StandardInterceptor());
		...
		{
			var invocationChecker = new AssertCannotChangeTargetInterceptor();
			var proxy = generator.CreateClassProxy(typeof(ClassWithCharRetType), invocationChecker);
			Assert.IsNotNull(proxy);
			var classProxy = (ClassWithCharRetType) proxy;
		...
		public void ProxyForBaseTypeAndInterfaceFromSignedAndUnsignedAssemblies1()
		{
			if (TestAssemblySigned())
				return;
			var t1 = typeof(MyClass);
			var t2 = typeof(IServiceProvider);
			Assert.IsFalse(t1.GetTypeInfo().Assembly.IsAssemblySigned());
			Assert.IsTrue(t2.GetTypeInfo().Assembly.IsAssemblySigned());
			var proxy = generator.CreateClassProxy(t1, new[] {t2}, new StandardInterceptor());
			Assert.IsFalse(proxy.GetType().GetTypeInfo().Assembly.IsAssemblySigned());
		}
		...
		public void ProxyForBaseTypeAndInterfaceFromSignedAndUnsignedAssemblies2()
		{
			...
			Assert.IsTrue(t1.GetTypeInfo().Assembly.IsAssemblySigned());
			Assert.IsFalse(t2.GetTypeInfo().Assembly.IsAssemblySigned());
			var proxy = generator.CreateClassProxy(t1, new[] {t2}, new StandardInterceptor());
			Assert.IsFalse(proxy.GetType().GetTypeInfo().Assembly.IsAssemblySigned());
		}
		...
		public void ProxyForBaseTypeAndInterfaceFromSignedAssembly()
		{
			...
			Assert.IsTrue(t2.GetTypeInfo().Assembly.IsAssemblySigned());
			var proxy = generator.CreateClassProxy(t1, new[] {t2}, new StandardInterceptor());
			Assert.AreEqual(shouldBeSigned, proxy.GetType().GetTypeInfo().Assembly.IsAssemblySigned());
		}
		...
		public void ProxyForBaseTypeFromSignedAssembly()
		{
			...
			Assert.IsTrue(t.GetTypeInfo().Assembly.IsAssemblySigned());
		...
		public void ProxyForBaseTypeFromUnsignedAssembly()
		{
			...
			Assert.IsFalse(t.GetTypeInfo().Assembly.IsAssemblySigned());
			var proxy = generator.CreateClassProxy(t, new StandardInterceptor());
			Assert.IsFalse(proxy.GetType().GetTypeInfo().Assembly.IsAssemblySigned());
		}
		...
		{
			var logger = new LogInvocationInterceptor();
			var proxy = generator.CreateClassProxy(typeof(ClassWithCharRetType), logger);
			Assert.IsNotNull(proxy);
			var classProxy = (ClassWithCharRetType) proxy;
		...
		public void ProxyForClass()
		{
			...
			Assert.AreEqual((uint) 45, instance.Sum(20, (uint) 25)); // uint
			Assert.AreEqual((ulong) 45, instance.Sum(20, (ulong) 25)); // ulong
		}
		...
		public void ProxyForClassWithIndexer()
		{
			...
			var proxy = generator.CreateClassProxy(typeof(ClassWithIndexer), logger);
